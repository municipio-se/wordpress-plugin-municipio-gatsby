...
class HeadlessMuncipioPostForm {
  public function __construct() {
    add_action("rest_api_init", [$this, "registerRestRoutes"]);
  }

  public function registerRestRoutes() {
    register_rest_route("modularity", "mod-form", [
      "methods" => "POST",
      "callback" => [$this, "postForm"],
    ]);
  }
  ...
  public function postForm($request) {
    ...
    $return = ["status" => "Error"];
    if (
      isset($params["modularity-form"]) &&
      wp_verify_nonce(
        $params["modularity-form"],
        "mod-form-" . $params["modularity-form-id"]
      )
    ) {
      $_POST = $params;
      $this->submit();
  ...
  public function submit() {
    ...
    $postTitle =
      !empty($_POST["post_title"]) && !empty($_POST[$_POST["post_title"]])
        ? $_POST[$_POST["post_title"]]
        : get_the_title($_POST["modularity-form-id"]);
    $postContent =
      !empty($_POST["post_content"]) && !empty($_POST[$_POST["post_content"]])
    ...
    if (empty($postReferer) || ($checkReferer !== 0 && $checkFormPage !== 0)) {
      $formUrl =
        ...
        "</a>";
      $refHistory =
        $postReferer !== null && $postReferer !== "null"
          ? "\r\n" .
            __("Previous page", "modularity-form-builder") .
            "<a href=\"" .
            $postReferer .
            "\" >" .
            $postReferer .
    ...
    update_post_meta(
      $submission,
      "modularity-form-id",
      $_POST["modularity-form-id"]
    );
    update_post_meta(
      $submission,
      "modularity-form-referer",
      strtok($referer, "?")
    );
    ...
    // Get correct labels
    $labels = \ModularityFormBuilder\Helper\SenderLabels::getLabels();
    ...
    foreach ($fields as $key => $field) {
      if ($field["acf_fc_layout"] == "sender") {
        if (!empty($field["custom_sender_labels"]["add_sender_labels"])) {
          $labels = array_merge(
            $labels,
            array_filter($field["custom_sender_labels"])
          );
        }
      }
    }
    // Get sender data
    $fromEmail = !empty($_POST[sanitize_title($labels["email"])])
      ? $_POST[sanitize_title($labels["email"])]
      : null;
    ...
    if ($notify) {
      foreach ($notify as $email) {
        $sendMail = true;
        if ($email["condition"]) {
          $conditionalFieldKey = sanitize_title(
            $email["form_conditional_field"]
          );
          $sendMail = false;
          if (array_key_exists($conditionalFieldKey, $_POST)) {
            if (
              $_POST[$conditionalFieldKey] ==
              $email["form_conditional_field_equals"]
            ) {
              $sendMail = true;
            }
          }
    ...
    foreach ($fileslist as $key => $files) {
      for ($i = 0; $i < count($files["name"]); $i++) {
        ...
        if (in_array("image/*", $fields[$key]["filetypes"])) {
          $fields[$key]["filetypes"] = array_unique(
            array_merge($fields[$key]["filetypes"], $allowedImageTypes)
          );
        }
        ...
        if (in_array("video/*", $fields[$key]["filetypes"])) {
          $fields[$key]["filetypes"] = array_unique(
            array_merge($fields[$key]["filetypes"], $allowedVideoTypes)
          );
        }
        ...
        if (
          ...
          empty($fields[$key]["upload_videos_external"])
        ) {
          if (
            defined("ENCRYPT_SECRET_VI") &&
            defined("ENCRYPT_SECRET_KEY") &&
            defined("ENCRYPT_METHOD")
          ) {
        ...
        if (move_uploaded_file($files["tmp_name"][$i], $targetFile)) {
          ...
          if (
            !empty($fields[$key]["upload_videos_external"]) &&
            in_array("." . $fileext, $allowedVideoTypes)
          ) {
            $uploadVideo = \ModularityFormBuilder\Helper\YoutubeUploader::uploadVideo(
              $targetFile,
              ucwords($fileName),
              "",
              "22"
  ...
  public function getMailDownloadLink($filePath) {
    //Check if encrypted
    if (
      strpos($filePath, sanitize_file_name("-enc-" . ENCRYPT_METHOD)) !== false
    ) {
      return home_url("/") .
        "?modFormDownloadEncFilePublic=" .
        urlencode(basename($filePath)) .
        "&token=" .
        md5(urlencode(basename($filePath)) . NONCE_KEY);
    }
  ...
  public function forceEcryptedPublicFileDownload() {
    if (isset($_GET["modFormDownloadEncFilePublic"]) && isset($_GET["token"])) {
      ...
      if (
        $_GET["token"] !==
        md5($_GET["modFormDownloadEncFilePublic"] . NONCE_KEY)
      ) {
        wp_die(
          __(
            "The download token provided was not correct.",
      ...
      $filePath =
        $uploadsFolder . urldecode($_GET["modFormDownloadEncFilePublic"]);
      ...
      if (isset($fileContents) && !empty($fileContents)) {
        ...
        header(
          "Content-Disposition: attachment; filename=\"" .
            $_GET["modFormDownloadEncFilePublic"] .
            "\";"
        );
  ...
  public static function getSubmissionData($submissionId): array {
    ...
    $formdata = [];
    $labels = \ModularityFormBuilder\Helper\SenderLabels::getLabels();
    $excludedFields = ["custom_content", "collapse"];
    foreach ($fields as $field) {
      if ($field["acf_fc_layout"] === "sender") {
    ...
    $formdata["modularity-form-history"] =
      $data["modularity-form-history"] ?? "";
  ...
  public function notify($email, $formId, $submissionId, $from = null) {
    ...
    $subject =
      get_field("notification_custom_subject", $formId) == true
          ...
          foreach ($value as $subvalue) {
            ...
            if (strpos($subvalue, "/modularity-form-builder/") !== false) {
              $message .=
                ...
                $this->getMailDownloadLink(
                  $uploadFolder . basename($subvalue)
                ) .
                '">' .
                basename($subvalue) .
                "</a>" .
          ...
          if ($key === "modularity-form-history") {
            $message .=
              $value !== null && $value !== "null"
                ? "<strong>" .
                  __("Referrer", "modularity-form-builder") .
                  "</strong><br>" .
                  $value
                : __("No Referrer", "modularity-form-builder");
          } else {
  ...
  public function sendCopy($email, $formId, $submissionId, $from = null) {
    ...
    $message = "";
    $subject =
      get_field("copy_custom_subject", $formId) == true
